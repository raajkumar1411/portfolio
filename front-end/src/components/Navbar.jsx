...
return(
    ...
    <li key={id} className='px-2 cursor-pointer capitalize text-sm text-gray-500 hover:scale-105  duration-200 hover:text-white'>{link}</li>
)
    })}
...
{links.map(({id,link})=>{
    return(
        ...
        hover:scale-105  duration-200 hover:text-white'>{link}</li>
        )
    })}
        </ul>
    )}
